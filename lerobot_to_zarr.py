...
class LeRobotDatasetConverter:
    ...
    def _process_image(self, images_tensor: torch.tensor) -> torch.tensor:
        ...
        return images_tensor.permute(0, 2, 3, 1)  # [B, H, W, C]
    ...
    def _extract_episode_data(self, episode_idx: int) -> Dict[str, np.ndarray]:
        ...
        for idx in range(episode_from_idx, episode_to_idx):
            ...
            if 'action' in episode_data:
                actions.append(episode_data['action'])
            
            if 'observation.state' in episode_data:
                states.append(episode_data['observation.state'])
                #print(episode_data['observation.state'])
            
            # Extract images
            if 'observation.images.front' in episode_data and 'observation.images.head_mask' in episode_data:
                ...
                mask = episode_data['observation.images.head_mask'].permute(1, 2, 0).numpy()
                if mask.max() <= 1.0:
                    mask = (mask * 255).astype(np.uint8)
                mask_gray = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)
                rgbm_img = np.concatenate([head_img, mask_gray[..., None]], axis=-1)
            ...
            if 'observation.images.wrist_left' in episode_data:
                left_img = episode_data['observation.images.wrist_left'].permute(1, 2, 0).numpy()
                if left_img.max() <= 1.0:
                    left_img = (left_img * 255).astype(np.uint8)
                left_cam_images.append(left_img) 
        ...
        return {
            ...
            'left_cam_img': np.array(right_cam_images, dtype=np.uint8),
    ...
    def convert(self):
        ...
        metadata = {
            ...
            'state_dim': buffer.data['state'].shape[1],
        ...
        print(f"State dimension: {buffer.data['state'].shape[1]}")
