...
class DexGraspVLAPolicy(BasePolicy):
    ...
    def infer(self, obs):
        ...
        mask = self.get_mask(obs['images']['cam_high'])
        obsm = self.get_obs(obs, mask)  # TODO: There might be misalignment between mask and image, needs verification
        attn_map_output_path = os.path.join(self.current_attn_maps_dir, f'{self.time_step}.pkl') if self.args.gen_attn_map else None
        self.time_step += 1
        obs_dict_np = self.process_obs(env_obs=obsm, shape_meta=self.cfg.task.shape_meta)
